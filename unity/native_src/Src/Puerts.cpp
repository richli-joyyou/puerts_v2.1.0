...
extern "C"
{
    ...
    V8_EXPORT v8::Isolate* CreateJSEngineWithExternalEnv(
        int backend, void* external_quickjs_runtime, void* external_quickjs_context)
    {
#if WITH_QUICKJS
    ...
    V8_EXPORT int _RegisterClass(v8::Isolate* Isolate, int BaseTypeId, const char* FullName,
        puerts::CSharpConstructorCallback Constructor, puerts::CSharpDestructorCallback Destructor, int64_t Data)
    {
        auto JsEngine = FV8Utils::IsolateData<JSEngine>(Isolate);
        return JsEngine->RegisterClass(FullName, BaseTypeId, Constructor, Destructor, Data, 0);
    }

    V8_EXPORT int RegisterStruct(v8::Isolate* Isolate, int BaseTypeId, const char* FullName,
        puerts::CSharpConstructorCallback Constructor, puerts::CSharpDestructorCallback Destructor, int64_t Data, int Size)
    {
        auto JsEngine = FV8Utils::IsolateData<JSEngine>(Isolate);
        return JsEngine->RegisterClass(FullName, BaseTypeId, Constructor, Destructor, Data, Size);
    }

    V8_EXPORT int RegisterFunction(
        v8::Isolate* Isolate, int ClassID, const char* Name, int IsStatic, puerts::CSharpFunctionCallback Callback, int64_t Data)
    {
        auto JsEngine = FV8Utils::IsolateData<JSEngine>(Isolate);
        return JsEngine->RegisterFunction(ClassID, Name, IsStatic, Callback, Data) ? 1 : 0;
    }

    V8_EXPORT int RegisterProperty(v8::Isolate* Isolate, int ClassID, const char* Name, int IsStatic,
        puerts::CSharpFunctionCallback Getter, int64_t GetterData, puerts::CSharpFunctionCallback Setter, int64_t SetterData,
        int DontDelete)
    {
        auto JsEngine = FV8Utils::IsolateData<JSEngine>(Isolate);
    ...
    V8_EXPORT bool PumpMessageLoop(v8::Isolate* Isolate)
    {
        auto JsEngine = FV8Utils::IsolateData<JSEngine>(Isolate);
        return JsEngine->PumpMessageLoop();
    }
    V8_EXPORT void PerformMicrotaskCheckpoint(v8::Isolate* Isolate)
    {
        auto JsEngine = FV8Utils::IsolateData<JSEngine>(Isolate);
        JsEngine->PerformMicrotaskCheckpoint();
    }
    ...
    V8_EXPORT JsValueType GetArgumentType(
        v8::Isolate* Isolate, const v8::FunctionCallbackInfo<v8::Value>& Info, int Index, int IsOut)
    {
        ...
        else
        {
            if (Value->IsFunction())
            {
                ...
                if (MaybeMap.IsEmpty())
                    return -1;
                auto MaybeValue =
                    v8::Local<v8::Map>::Cast(MaybeMap.ToLocalChecked())->Get(Context, FV8Utils::V8String(Isolate, "classid"));
                if (MaybeValue.IsEmpty())
                    return -1;
    ...
    V8_EXPORT void ReturnArrayBuffer(
        v8::Isolate* Isolate, const v8::FunctionCallbackInfo<v8::Value>& Info, unsigned char* Bytes, int Length)
    {
    ...
    V8_EXPORT void ReturnCSharpFunctionCallback(v8::Isolate* Isolate, const v8::FunctionCallbackInfo<v8::Value>& Info,
        puerts::CSharpFunctionCallback Callback, int64_t Data)
    {
        auto JsEngine = FV8Utils::IsolateData<JSEngine>(Isolate);
#ifdef THREAD_SAFE
