...
#endif
{
    ...
    {
        Global
            ->Set(Context, FV8Utils::V8String(Isolate, "__puertsGetLastException"),
                v8::FunctionTemplate::New(Isolate, &GetLastException)->GetFunction(Context).ToLocalChecked())
            .Check();
    }
    Global
        ->Set(Context, FV8Utils::V8String(Isolate, "__tgjsEvalScript"),
            v8::FunctionTemplate::New(Isolate, &EvalWithPath)->GetFunction(Context).ToLocalChecked())
        .Check();

    JSObjectIdMap.Reset(Isolate, v8::Map::New(Isolate));

    JSObjectValueGetter = CreateJSFunction(
        Isolate, Context, v8::FunctionTemplate::New(Isolate, &JSObjectValueGetterFunction)->GetFunction(Context).ToLocalChecked());
...
JSFunction* JSEngine::GetModuleExecutor()
{
    if (ModuleExecutor == nullptr)
    {
        ...
        {
            ModuleExecutor = CreateJSFunction(MainIsolate, Context, Func.As<v8::Function>());
        }
    }
...
#ifdef MULT_BACKENDS
        Function =
            new JSFunction(ResultInfo.PuertsPlugin, InIsolate, InContext, InFunction, static_cast<int32_t>(JSFunctions.size()));
#else
...
v8::Local<v8::FunctionTemplate> JSEngine::ToTemplate(
    v8::Isolate* Isolate, bool IsStatic, CSharpFunctionCallback Callback, int64_t Data)
{
...
void JSEngine::SetGlobalFunction(const char* Name, CSharpFunctionCallback Callback, int64_t Data)
{
    ...
    Global
        ->Set(Context, FV8Utils::V8String(Isolate, Name),
            ToTemplate(Isolate, true, Callback, Data)->GetFunction(Context).ToLocalChecked())
        .Check();
}
...
static void NewWrap(const v8::FunctionCallbackInfo<v8::Value>& Info)
{
    ...
    if (Info.IsConstructCall())
    {
        ...
        {
#ifdef MULT_BACKENDS
            if (LifeCycleInfo->Constructor)
                Ptr = LifeCycleInfo->Constructor(JsEngine->ResultInfo.PuertsPlugin, Info, Info.Length(), LifeCycleInfo->Data);
#else
            if (LifeCycleInfo->Constructor)
                Ptr = LifeCycleInfo->Constructor(Isolate, Info, Info.Length(), LifeCycleInfo->Data);
#endif
        }
...
int JSEngine::RegisterClass(const char* FullName, int BaseClassId, CSharpConstructorCallback Constructor,
    CSharpDestructorCallback Destructor, int64_t Data, int Size)
{
...
bool JSEngine::RegisterFunction(int ClassID, const char* Name, bool IsStatic, CSharpFunctionCallback Callback, int64_t Data)
{
    ...
    {
        Templates[ClassID].Get(Isolate)->PrototypeTemplate()->Set(
            FV8Utils::V8String(Isolate, Name), ToTemplate(Isolate, IsStatic, Callback, Data));
    }

    return true;
}

bool JSEngine::RegisterProperty(int ClassID, const char* Name, bool IsStatic, CSharpFunctionCallback Getter, int64_t GetterData,
    CSharpFunctionCallback Setter, int64_t SetterData, bool NotReadonlyStatic)
{
    v8::Isolate* Isolate = MainIsolate;
#ifdef THREAD_SAFE
    v8::Locker Locker(Isolate);
...
bool JSEngine::PumpMessageLoop()
{
#ifndef WITH_QUICKJS
    v8::Isolate::Scope Isolatescope(MainIsolate);
    v8::HandleScope HandleScope(MainIsolate);
    return v8::platform::PumpMessageLoop(BackendEnv._platform, MainIsolate, v8::platform::MessageLoopBehavior::kDoNotWait);
#endif
    return true;
}

void JSEngine::PerformMicrotaskCheckpoint()
{
#ifndef WITH_QUICKJS
    v8::Isolate::Scope Isolatescope(MainIsolate);
    v8::HandleScope HandleScope(MainIsolate);
    if (BackendEnv._platform->IdleTasksEnabled(MainIsolate))
    {
        v8::platform::RunIdleTasks(BackendEnv._platform, MainIsolate, 20);
    }
#endif
}
